...
if ($_SERVER["REQUEST_METHOD"] == "GET" && isset($_GET["pizza"])) {
    ...
    $conn = new PDO('pgsql:host=localhost;dbname=pizzaria_teste', 'postgres', 'postgres');

    // Define o modo de erro para exceções
    $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);

    // Prepara a consulta SQL com um marcador de posição para o sabor da pizza
    $stmt = $conn->prepare('SELECT * FROM contatos WHERE pizza = :pizza');

    // Executa a consulta com o pizza da pizza fornecido
    $stmt->execute(['pizza' => $_GET["pizza"]]);
